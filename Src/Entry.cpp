  ...
  void Setup() 
  {
    ...
    { 
      ...
      else if(reinterpret_cast<size_t>(writeDataDest) >= 0x801E000 && reinterpret_cast<size_t>(writeDataDest) <= 0x8020000)
      {
        ...
        pOBInit.WRPPage = OB_WRP_PAGES120TO123;
        ...
        if(reinterpret_cast<size_t>(writeDataDest) == 0x801E000)
        {
          ...
          pEraseInit.PageAddress = reinterpret_cast<uint32_t>(&_config);

          uint32_t SectorError;
          HAL_FLASHEx_Erase(&pEraseInit, &SectorError);
        }

        for(size_t i = 0; i < writeDataLength; i+=2)
        {
          uint32_t programType = FLASH_TYPEPROGRAM_HALFWORD;
          uint16_t data = 0;
          std::memcpy(&data,  reinterpret_cast<void *>(reinterpret_cast<uint8_t *>(writeData) + i), sizeof(uint16_t));
